...
const Wishlist = () => {
  ...
  const handleAddtoCart = (id: number, wishlistId: number) => {
    let quantity = 1;
  ...
  const handleAddAllToCart = () => {
    ...
    const activeItems = wishlistItems.filter(
      (item: WishlistDataType) => item.product.status === "active"
    );

    activeItems.forEach((item: WishlistDataType) => {
      const cartData = {
        id: item.product.id,
        quantity,
      } as CartAddDataType;
      cartAddMutation(cartData);
                        ...
                        <div className="text-center flex justify-center gap-5 items-center">
                          {item.product.status === "active" && (
                            <button
                              ...
                              onClick={() =>
                                handleAddtoCart(item.product.id, item.id)
                              }
                            >
                              <img
